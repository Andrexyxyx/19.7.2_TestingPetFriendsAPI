from app.PetFriends_API import PetFriends
from app.PetFriends_Settings import valid_email, valid_password, invalid_email, invalid_password
import os
...
def test_add_new_pet_with_valid_data(name = 'Rocky', animal_type = 'tiger', age = '5', pet_photo = 'images/tiger.jpg'):
...
def test_delete_pet_successful():
    ...
    if len(my_pets['pets']) == 0: # Проверяем - если список своих питомцев пустой, то добавляем нового и опять запрашиваем список своих питомцев
        PetFriends().add_new_pet(auth_key, 'Enot', 'racoon', '11', 'images/racoon.jpg')
...
def test1_add_new_pet_without_photo_with_valid_data(name = 'AlexWithouthPhoto', animal_type = 'bear', age = '7'):
...
def test2_add_photo_of_pet(pet_photo = 'images/racoon1.jpg'):
...
def test6_add_new_pet_with_empty_name(name = '', animal_type = 'tiger', age = '5', pet_photo = 'images/tiger.jpg'):
...
def test7_add_new_pet_with_special_symbols_name(name = 'Max#@$', animal_type = 'tiger', age = '5', pet_photo = 'images/tiger.jpg'):
    ...
    assert status != 200 # Ожидаем, что код статуса запроса будет не равен 200
    assert result['name'] != 'Max#@$' # Ожидаем, что питомец не будет добавлен на сайт со спецсимволами в поле name


def test8_add_new_pet_with_empty_animal_type(name = 'Tigra', animal_type = '', age = '5', pet_photo = 'images/tiger.jpg'):
...
def test9_add_new_pet_with_special_symbols_animal_type(name = 'Tigra', animal_type = 'tiger#@$', age = '5', pet_photo = 'images/tiger.jpg'):
    ...
    status, result = PetFriends().add_new_pet(auth_key, name, animal_type, age, pet_photo)
    assert status != 200 # Ожидаем, что код статуса запроса будет не равен 200
    assert result['animal_type'] != 'tiger#@$' # Ожидаем, что питомец не будет добавлен на сайт со спецсимволами в поле name


def test10_add_new_pet_negative_age(name = 'Tigra', animal_type = 'tiger', age = '-5', pet_photo = 'images/tiger.jpg'):
